...
* 设置全局参数
*/
std::string scan_ip_from="173.0.0.5";//另外一组节点的IP起始地址
std::string scan_ip_to="173.0.0.254";//另外一组节点的IP结束地址
uint16_t scan_port=5000;             // 接受端口
float scan_interval=2.0;             //发包间隔时间
std::string local_network="171.0.0.0";//表明当前节点的网络地址
int node_number=250;                 //模拟节点个数
std::string KVM_gateway_address="171.0.0.1";//本网络节点的网关
...
//设置种子为1
 ...
 uv->SetAttribute("Max",DoubleValue(range));
  ...
  for(i=0;i<node_number;i++){
      ...
      interfacep[i]=csmaInterfaces.Get(i).second;//一个是地址 一个是索引 second就是那个索引
	  //设置跳数
 ...
 //设置node的路由，指向对应kvm路由器的地址
  ...
  /*
  *接下来的思路是：选择第二个计算节点随机向一个计算节点x疯狂发送数据包，
  */
   ...
   Ptr<Ipv4> ipv41 = csmaNodes.Get(1)->GetObject<Ipv4>();
   Ipv4InterfaceAddress iaddr1 = ipv41->GetAddress(1, 0);
   Ipv4Address ipaddr1 = iaddr1.GetLocal();
   std::cout << localip << " " << Simulator::Now().GetSeconds() << " " << ipaddr1 << " " << ipaddr1 << std::endl;
   Ptr<Socket> srcSocket;
   srcSocket = Socket::CreateSocket (csmaNodes.Get(1), TypeId::LookupByName ("ns3::UdpSocketFactory"));
          // 调用发送函数 getRandStrIpv4().c_str() 修改成dest_IP
   Simulator::ScheduleWithContext (srcSocket->GetNode()->GetId (),
                                  Seconds (scan_interval), &SendStuff, 
                                  srcSocket, Ipv4Address(dest_IP), scan_port);
